"""
Dokumentace k uživatelskému rozhraní

Tato část projektu obsahuje třídu pro uživatelské rozhraní (UI), která umožňuje interakci s databází e-shopu prostřednictvím příkazového řádku.

Třídy zahrnují:

1. UI - Třída pro uživatelské rozhraní.
"""
...
class UI:
    """
    Třída UI je zodpovědná za poskytování uživatelského rozhraní pro interakci s databází e-shopu.

    Metody:
    - __init__(): Inicializuje tovární třídy pro správu databázových operací.
    - menu(): Hlavní menu aplikace.
    - customer_menu(): Menu pro správu zákazníků.
    - product_menu(): Menu pro správu produktů.
    - order_menu(): Menu pro správu objednávek.
    - order_item_menu(): Menu pro správu položek objednávek.
    - transaction_menu(): Menu pro správu transakcí.
    """
    
    def __init__(self):
        # Inicializace továrních tříd pro správu databázových operací
        ...
        while True:
            ...
            if choice == "1":
                ...
                if transaction:
                    ...
                    print("Credit Points: ", transaction[3])
                else:
